...
```{r}
resultado_b <- Datos_Banco %>%
  filter(Mes %in% c("marzo", "abril")) %>%
  group_by(Mes, aceptacion_TC) %>%
...
resultado_c <- Datos_Banco %>%
  group_by(aceptacion_TC) %>%
  summarise(Skewness = skewness(contrato_meses))
...
Datos_Banco %>%
  ...
  facet_wrap(~aceptacion_TC) +
  theme_minimal() +
  ggtitle("Distribución de Meses de Contrato por Aceptación de Tarjeta")
